...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://rzksnyklqwiuwv:cb5fa4596734158c03d6f9327947289570aadd658eae1c8f2d971ad9241bfd3d@ec2-54-147-107-18.compute-1.amazonaws.com:5432/d9dha9u21gveg6'
...
def registrar_editorial():
    ...
    db.session.commit()
    return render_template("menuEditorialGenero.html")
...
@app.route("/menuautor")
def menuautor():
    return render_template("menuautor.html")

@app.route("/menuEditorialGenero")
def menuEditorialGenero():
    return render_template("menuEditorialGenero.html")

@app.route("/menuLibro")
def menuLibro():
    return render_template("menuLibro.html")
...
##########################################################
@app.route("/editorial_lista")
def editorial_lista():
    consulta_editorial = Editorial.query.all()    
    return render_template("listaeditorial.html", consulta = consulta_editorial)

@app.route("/eliminar_editorial/<id>")
def eliminar_editorial(id):
    editorial=Editorial.query.filter_by(id_editorial=int(id)).delete()
    print(editorial)
    db.session.commit()
    return redirect("/editorial_lista")
...
@app.route("/editar_editorial/<id>")
def editar_editorial(id):
    consulta_editorial = Editorial.query.all()
    editorial = Editorial.query.filter_by(id_editorial=int(id)).first()
    db.session.commit()
    return render_template("modificareditorial.html",editorial=editorial,consulta_editorial=consulta_editorial)


################################################################
@app.route("/autor_lista")
def autor_lista():
    consulta_autor = Autor.query.all()    
    return render_template("listaautor.html", consulta = consulta_autor)

@app.route("/eliminar_autor/<id>")
...
@app.route("/editar_autor/<id>")
def editar_autor(id):
    consulta_autor = Autor.query.all()
    autor = Autor.query.filter_by(id_autor=int(id)).first()
    db.session.commit()
    return render_template("modificarautor.html",autor=autor,consulta_autor=consulta_autor)
...
##############################################################################
@app.route("/genero_lista")
def genero_lista():
    consulta_genero =Genero.query.all()    
    return render_template("listagenero.html", consulta = consulta_genero)

@app.route("/eliminar_genero/<id>")
...
@app.route("/editar_genero/<id>")
def editar_genero(id):
    consulta_genero=Genero.query.all()
    genero=Genero.query.filter_by(id_genero=int(id)).first()
    db.session.commit()
    return render_template("modificargenero.html",genero=genero,consulta_genero=consulta_genero)
